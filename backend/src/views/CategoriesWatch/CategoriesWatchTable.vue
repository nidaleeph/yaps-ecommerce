    ...
    <table class="table-auto w-full">
      ...
      <tbody v-else>
        <tr v-for="(category, index) of categories.data">
          ...
          <td class="border-b p-2">
            {{ moment(category.created_at).format("MMMM D, YYYY h:mm:ss a") }}
          </td>
          <td class="border-b p-2">
...
const categories = computed(() => store.state.categoriesWatch);
...
function getForPage(ev, link) {
  ...
  getCategoriesWatch(link.url);
}

function getCategoriesWatch(url = null) {
...
function sortCategoriesWatch(field) {
...
function deleteCategory(category) {
  ...
  store.dispatch("deleteCategoryWatch", category).then((res) => {
