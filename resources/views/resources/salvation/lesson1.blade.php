<x-app-layout>
    ...
    <body class="bg-fixed" onload="defaultbank()"
        ...
        <div class="h-screen flex flex-col items-center justify-center mx-4">
            <div id="player" class="md:w-3/4 lg:w-3/4 xl:w-3/4 lg:h-3/5 xl:h-3/4 h-2/4"></div>
            <button id="redirectButton" style="display: none;" onclick="redirectToPage()"
                class="mt-4 bg-blue-500 text-white px-4 py-2 rounded">Redirect</button>
        </div>
        <script>
            let player;
            const videoId = 'FAyKDaXEAgc';
            ...
            function handleVisibilityChange() {
                ...
                // else {
                //     // Resume playing when the tab becomes visible again
                //     player.playVideo();
                // }
            }
        </script>
        <div class="flex flex-col pt-5 bg-gray-200 text-center opacity-90">
        ...
        <div class="flex flex-col pt-5 bg-gray-300 text-center opacity-90">
            ...
            <form action="{{ route('cart.checkoutCustom') }}" method="post" enctype="multipart/form-data">
                ...
                <button type="submit"
                    class="bg-cyan-600 hover:bg-blue-200 text-white rounded-full py-3 px-12 font-poppins font-bold text-1xl mb-4 slide-up">
                    Submit
                </button>
            </form>
