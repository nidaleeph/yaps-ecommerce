    ...
    <div x-data="productItem({{ json_encode([
        ...
        <div class="grid gap-6 grid-cols-1 lg:grid-cols-5">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="text-xl font-bold mb-6 @if ($activeEvent) line-through @endif">
                    ₱{{ $product->price }}</div>
                @if ($activeEvent)
                    <div class="text-xl font-bold error-message">{{ $activeEvent->name }}:
                        ₱{{ number_format($product->price - $product->price * ($activeEvent->percentage / 100), 2) }}
                    </div>
                @endif
                ...
                @elseif ($product->quantity >= 0)
                    <div class="text-xl  mb-4 "><span class="font-bold">Stock: </span><span
                            class="error-message">{{ $product->quantity }}</span></div>
                @endif
                ...
                <button :disabled="product.quantity === 0" @click="addToCart($refs.quantityEl.value)"
                    class="btn-primary bg-cyan-600 py-4 text-lg flex justify-center min-w-0 w-full mb-6"
